...
const char HTTP_OFFLINE[] PROGMEM           = "<div>Starting offline operation!<br />You can close the interface.</div>";
...
class WiFiManager
{
  public:
    ...
    int           autoConnect();
    int           autoConnect(char const *apName, char const *apPassword = NULL);

    //if you want to always start the config portal, without trying to connect first
    boolean       startConfigPortal();
    boolean       startConfigPortal(char const *apName, char const *apPassword = NULL);
	boolean       printOffline();
  ...
  private:
    ...
    std::unique_ptr<ESP32WebServer>        server;
